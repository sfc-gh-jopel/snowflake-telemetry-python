...
class ProtoSerializer:
    ...
    def serialize_message(
        ...
        self.out += tag
        self.out.extend(b"\x00\x00\x00\x00\x00")
        start = len(self.out)
        value.write_to(self)
        size = len(self.out) - start
        start -= 5
        self.out[start] = (size & 0x7F | 0x80)
        self.out[start + 1] = ((size >> 7) & 0x7F | 0x80)
        self.out[start + 2] = ((size >> 14) & 0x7F | 0x80)
        self.out[start + 3] = ((size >> 21) & 0x7F | 0x80)
        self.out[start + 4] = (size >> 28)
